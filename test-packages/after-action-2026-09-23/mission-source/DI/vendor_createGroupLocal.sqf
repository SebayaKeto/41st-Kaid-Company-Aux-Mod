...
// DI instrumentation: explicit creation identity before any units exist.
_group setVariable ["DI_operation",_diAction select 0,true];
_group setVariable ["DI_action",_diAction select 1,true];
_group setVariable ["DI_nonce",_diAction select 2,true];
_group setVariable ["DI_objective",_diAction select 3,true];
_group setVariable ["DI_lease",1,true];
_group setVariable ["DI_humanUntil",0,true];
...
if (count _vehData > 0) then {
    ...
    if (count _unitData > 0) then {
        ...
        {
            private _offset = [(_pos select 0) + random 10 - 5, (_pos select 1) + random 10 - 5, _pos select 2];
...
// DI caller receives the exact group, never a nearest/newest-group guess.
_group
