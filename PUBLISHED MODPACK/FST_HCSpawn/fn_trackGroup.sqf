...
// Arguments:
//   0: GROUP or STRING (netId)
//   1: NUMBER - HC index (ignored when arg 5 is a valid owner ID)
//   2: BOOL   - true if FST_HC_UnitCounts was already pre-incremented
//   3: NUMBER - retry attempt (internal)
//   4: NUMBER - spawn protection seconds
//   5: NUMBER - HC owner ID (V27, preferred). HC indexes shift when an HC
//               disconnects between dispatch and track; owner IDs do not.

if (!isServer) exitWith {};

params ["_group", ["_hcIndex", -1], ["_preCounted", false], ["_attempt", 0], ["_protectionSeconds", 30], ["_hcOwnerId", -1]];
...
if (isNull _group) exitWith {
    if (_attempt < 10) then {
        [{
            params ["_groupRef", "_hcIndex", "_preCounted", "_attempt", "_protectionSeconds", "_hcOwnerId"];
            [_groupRef, _hcIndex, _preCounted, _attempt + 1, _protectionSeconds, _hcOwnerId] call FST_HCSpawn_fnc_trackGroup;
        }, [_groupRef, _hcIndex, _preCounted, _attempt, _protectionSeconds, _hcOwnerId], 0.5] call CBA_fnc_waitAndExecute;
    } else {
        diag_log format ["[FST_HCSpawn] TrackGroup failed: unresolved group ref %1 for HC index %2 / owner %3", _groupRef, _hcIndex, _hcOwnerId];
        [] call FST_HCSpawn_fnc_recountUnits;
    };
};

if (_hcOwnerId > 2) then {
    _hcIndex = FST_HC_Ids find _hcOwnerId;
};
...
// Only broadcast the clear if the marker was actually set; every track used to
// publicVariable this to all clients regardless.
...
if (!_wasTracked) then {
    ...
    if (_oldIdx != _hcIndex) then {
        ...
        // Same HC, unit count changed. Reconcile without waiting for the periodic recount.
        if (!_preCounted) then {
