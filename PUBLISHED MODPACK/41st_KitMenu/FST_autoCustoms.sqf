...
private _kittaglower  = _kitTagLower;
...
private _restoreUniformItems = {
    params ["_unit","_items"];
    { _unit addItemToUniform _x; } forEach _items;
};
private _restoreVestItems = {
    params ["_unit","_items"];
    { _unit addItemToVest _x; } forEach _items;
};
private _restoreBackpackItems = {
    params ["_unit","_items"];
...
if (_allowHelmet) then {
    ...
    if ((count _helms) == 1) then {
        ...
        if ((count _helms) > 1) then {
            ...
            if (_chosen != "") then { removeHeadgear _unit; _unit addHeadgear _chosen; };
        };
    };
};
if (_allowArmor) then {
...
if (_allowNVG) then {
    private _nvgPrefixes = [ format ["FST_NVG_%1", _playerName] ];
    ...
    if ((count _nvgs) == 1) then {
        ...
        if ((count _nvgs) > 1) then {
            ...
            if (_chosen != "") then { _unit linkItem _chosen; };
        };
    };
};
if (_allowBack) then {
