...
// - switchMove / playMoveNow have global effect; remoteExec-to-all copies replaced
//   with the direct command (one network message per client per call before).
// - nearestObjects -> nearEntities for every "Man"/BX scan.
// - The always-on "is there a lightsaber enemy nearby" loop keeps its 0.2 s
//   poll (reaction time matters) but uses the cheaper nearEntities scan.
// - Per-frame handlers exit early once the droid is dead/deleted.
...
_unitWithSword = _this;
_countUnitsAround = {!(_x == _unitWithSword) and (alive _x) and !(lifeState _x == "INCAPACITATED")} count (_unitWithSword nearEntities ["WBK_BX_Assasin_1", 15]);
...
} forEach (_unitWithSword nearEntities ["WBK_BX_Assasin_1", 15]);
...
} forEach (_unit nearEntities ["WBK_BX_Assasin_1", 20]);
...
_main switchMove "SM_Sword_Execution";
_victim switchMove "SM_Sword_Execution_Victim";
_victim switchMove "SM_Sword_Execution_Victim";
...
detach _victim;
_main switchMove "AmovPercMstpSlowWrflDnon";
...
_main setVariable ["AI_CanTurn",1];
_main setVariable ["actualSwordBlock",0, true];
_main switchMove "Human_Execution_GenericOnehanded_headSmash_1_main";
_victim switchMove "Human_Execution_GenericOnehanded_headSmash_1_victim";
_victim switchMove "Human_Execution_GenericOnehanded_headSmash_1_victim";
[_main, "Disable_Gesture"] remoteExec ["playActionNow", _main];
[_victim, "Disable_Gesture"] remoteExec ["playActionNow", _victim];
...
_main setVariable ["AI_CanTurn",1];
_main setVariable ["actualSwordBlock",0, true];
_main switchMove "SM_Fists_Execution_main";
_victim switchMove "SM_Fists_Execution_victim";
_victim switchMove "SM_Fists_Execution_victim";
[_main, "Disable_Gesture"] remoteExec ["playActionNow", _main];
[_victim, "Disable_Gesture"] remoteExec ["playActionNow", _victim];
_victim attachTo [_main,[0,0,0]];
...
_actFrWeapExecut = [{
	...
	} forEach (_mutant nearEntities ["Man", 10]);
...
while {((_waitTillGuyAppear) and (alive _unitWithSword) and (local _unitWithSword))} do {
{
...
} forEach (_unitWithSword nearEntities ["Man", 15]);
sleep 0.2;
};
...
if (!(local _unitWithSword)) exitWith {};
...
_unitWithSword switchMove "melee_armed_idle";
...
_droid switchMove "SW_FIGHT_DROID_COMMANDO_FIRING";
sleep 0.6;
...
sleep 0.8;
if (!(animationState _droid == "SW_FIGHT_DROID_COMMANDO_FIRING")) exitWith {};
_droid switchMove "melee_armed_idle";
...
_droid = _this;
_droid switchMove "SW_FIGHT_DROID_COMMANDO_DODGING";
...
} forEach (_this nearEntities ["Man", 7]);
sleep 0.1;
_this setDamage 1;
};
...
_unitToPlay switchMove "starWars_lightsaber_lightattack3";
...
_actFr = [{
	...
	if (_mutant checkAIFeature "MINEDETECTION" || {_mutant checkAIFeature "WEAPONAIM"} || {_mutant checkAIFeature "SUPPRESSION"} || {_mutant checkAIFeature "COVER"} || {_mutant checkAIFeature "AIMINGERROR"} || {_mutant checkAIFeature "TARGET"} || {_mutant checkAIFeature "AUTOCOMBAT"} || {_mutant checkAIFeature "FSM"}) then {
	...
	} forEach (_mutant nearEntities ["Man", 40]);
	_myNearestEnemy = _mutant findNearestEnemy _mutant;
...
_unit = _array select 0;
if (isNull _unit || {!alive _unit} || {!local _unit}) exitWith { [_this select 1] call CBA_fnc_removePerFrameHandler; };
_CtnUn = {(!(side _x == side _unit) and (handGunWeapon _x in IMS_Melee_Weapons) and (alive _x) and ((getPosATL _x select 2) < 1.45))} count (_unit nearEntities ["Man", 15]);
if (_CtnUn == 0) then {
...
(!(alive _unitWithSword) or !(local _unitWithSword) or (_unitWithSword getVariable "IMS_Droid_SwitchToNormalWeapon" == 1))
};
...
[_unitWithSword, "ims_takeOut_TwoHanded_sheat"] remoteExec ["playActionNow", _unitWithSword];
sleep 0.7;
if (!(alive _unitWithSword) or !(local _unitWithSword)) exitWith {};
[_unitWithSword, "generic_out", 20, 6] execVM "\WebKnight_StarWars_Mechanic\createSoundGlobal.sqf";
_unitWithSword switchMove "AmovPercMstpSrasWrflDnon";
