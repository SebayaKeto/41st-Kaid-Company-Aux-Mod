class CfgPatches
{
	class FST_Relbyk23_Pistol
	{
		requiredAddons[]=
		{
			"A3_Weapons_F"
		};
		units[]={};
		weapons[]={};
	};
};
class Mode_SemiAuto;
class Mode_FullAuto;
class CfgWeapons
{
	class JMSLLTE_relbyk23pistol;
	class FST_Relbyk25_Pistol: JMSLLTE_relbyk23pistol
	{
		author="Gold";
		scope=2;
		displayName="[41st] Relby K-25 blaster pistol";
		baseWeapon="FST_Relbyk23_Pistol";
		model="\JMSLLTE_weapons\relbyk23\relbyk23.p3d";
		magazines[]=
		{
			"FST_blaster_cell_SSP_Blue",
			"FST_blaster_cell_SSP_Green",
			"FST_blaster_cell_SSP_Yellow",
			"FST_blaster_cell_SSP_Red"
		};
		class Single: Mode_SemiAuto
		{
			sounds[]=
			{
				"StandardSound"
			};
			class BaseSoundModeType;
			class StandardSound: BaseSoundModeType
			{
				soundSetShot[]=
				{
					"JMSLLTE_relbyk23_Shot_SoundSet",
					"SDAR_Tail_SoundSet",
					"SDAR_InteriorTail_SoundSet"
				};
			};
			recoil="recoil_pistol_heavy";
			recoilProne="recoil_prone_pistol_heavy";
			reloadTime=0.13;
			dispersion=0.00013499999;
			minRange=2;
			minRangeProbab=0.5;
			midRange=100;
			midRangeProbab=0.69999999;
			maxRange=10000;
			maxRangeProbab=0.30000001;
		};
	};
};
