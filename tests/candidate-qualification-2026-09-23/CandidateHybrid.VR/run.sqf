...
["hybrid preset applied",BURNS_ArmorAssistEnabled]call Q_check;
if(!BURNS_ArmorAssistEnabled)exitWith{diag_log "[QUAL_ABORT] requested safe setting was not applied"};
["hybrid preset four HCs",count(missionNamespace getVariable["FST_HC_Ids",[]])==4]call Q_check;
diag_log "[QUAL_HYBRID_ARMOR_BEGIN]";
{
 private _task=_x;
 {
  ...
  private _label=format["hybrid AAT %1 bearing %2",_task,_bearing];
