...
if (abs speed _v>12) exitWith {
    ...
    if ((velocityModelSpace _v select 1)<-1 && {abs _angle>90}) then {
        ...
        _v setVariable ["BURNS_armorTurnNext",time+0.5];
    };
};
...
// End when the hull has actually turned enough, including crossing the target
// bearing, instead of holding a fixed-duration turn through overshoot.
[{params ["_v","_token","_target","_initial"];
    if (isNull _v || {!local _v} || {!alive _v} || {isNull _target}) exitWith {true};
    private _pulse=_v getVariable ["BURNS_armorPulse",[]];
    if (count _pulse==0 || {(_pulse select 0)!=_token}) exitWith {true};
    private _remaining=(((_v getDir _target)-getDir _v+540) mod 360)-180;
    abs _remaining<25 || {_remaining*_initial<=0}
},FST_HCSpawn_fnc_burnsArmorPulseEnd,[_v,_token,_target,_angle],0.4,FST_HCSpawn_fnc_burnsArmorPulseEnd] call CBA_fnc_waitUntilAndExecute;
