...
diag_log "[QUAL_PODS_BEGIN]";Q_aatHandle=[]execVM "aat.sqf";
...
// Unmodified external creature class: check whether blood particle errors also
// occur without Gulanthar-specific gait selection.
private _g=createGroup[east,true];private _tg=createGroup[west,true];
private _u=_g createUnit["dev_form939_o",[14500,16000,0],[],0,"NONE"];
private _t=_tg createUnit["B_Soldier_F",[14500,16003,0],[],0,"NONE"];
_u allowDamage false;_t allowDamage false;_t disableAI "ALL";_g reveal[_t,4];
diag_log "[QUAL_EXTERNAL_CREATURE_CONTROL_BEGIN]";sleep 15;
diag_log "[QUAL_EXTERNAL_CREATURE_CONTROL_END]";
{deleteVehicle _x}forEach[_u,_t];deleteGroup _g;deleteGroup _tg;
waitUntil{sleep 1;scriptDone Q_aatHandle};diag_log format["[QUAL_FOLLOWUP_DONE] checks=%1 failures=%2",count Q_checks,Q_checks select{!(_x select 1)}];
