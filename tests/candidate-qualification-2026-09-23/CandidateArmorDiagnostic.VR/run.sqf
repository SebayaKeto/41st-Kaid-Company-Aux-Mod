...
["diagnostic preset applied",BURNS_ArmorAssistEnabled]call Q_check;
if(!BURNS_ArmorAssistEnabled)exitWith{diag_log "[QUAL_ABORT] requested safe setting was not applied"};
["diagnostic preset four HCs",count(missionNamespace getVariable["FST_HC_Ids",[]])==4]call Q_check;
diag_log "[QUAL_ARMOR_DIAGNOSTIC_BEGIN]";
{
 private _frozen=_x;
 {
 private _task=_x;
 {
  ...
  if(_frozen)then{_t enableSimulationGlobal false};
  ...
  for "_j" from 1 to 70 do{sleep 0.5;_samples pushBack[abs((((_v getDir _t)-getDir _v+540)mod 360)-180),velocityModelSpace _v,getPosATL _v,getPosATL _t,([_v,"VIEW",_t] checkVisibility[eyePos gunner _v,aimPos _t]),assignedTarget gunner _v,canFire _v,currentCommand driver _v]};
  private _label=format["diagnostic AAT %1 bearing %2 frozen %3",_task,_bearing,_frozen];
  diag_log format["[QUAL_TARGET_PHYSICS] %1",[_label,_t distance2D _targetStart,getPosATL _t,velocity _t]];
