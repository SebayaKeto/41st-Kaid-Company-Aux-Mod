...
["safe preset applied",!BURNS_ArmorAssistEnabled]call Q_check;
if(BURNS_ArmorAssistEnabled)exitWith{diag_log "[QUAL_ABORT] requested safe setting was not applied"};
["safe preset four HCs",count(missionNamespace getVariable["FST_HC_Ids",[]])==4]call Q_check;
diag_log "[QUAL_SAFE_ARMOR_BEGIN]";
{
 private _task=_x;
 {
  ...
  for "_j" from 1 to 40 do{sleep 0.5;_samples pushBack[abs((((_v getDir _t)-getDir _v+540)mod 360)-180),velocityModelSpace _v]};
  private _label=format["safe AAT %1 bearing %2",_task,_bearing];
  [_label+" firing",count(_v getVariable["Q_shots",[]])>0,[_v getVariable["Q_shots",[]],_v distance2D _start]]call Q_check;
  private _late=_samples select[20,20];
  [_label+" front presented",{(_x select 0)<55}count _late>=15,[{(_x select 0)<55}count _late,_samples]]call Q_check;
  [_label+" no turn lease",count(_v getVariable["BURNS_armorPulse",[]])==0]call Q_check;
