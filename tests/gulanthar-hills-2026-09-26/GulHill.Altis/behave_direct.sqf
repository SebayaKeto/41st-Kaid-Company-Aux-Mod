    ...
    while {!isNull _unit && {alive _unit} && {local _unit} && {(_unit getVariable ["FST_varrenToken", -1]) == _token}} do {
        ...
        if (_mode > 0 && {!isNull _target} && {[_unit, _token] call FST_Gulanthar_fnc_valid}) then {
            if (time >= _nextMove) then {
                ...
                if (_mode == 2) then {
                    // Steer straight at the target without path planning; fall back to a
                    // planned order only when direct steering stalls.
                    private _tpos = getPosATL _target;
                    private _planUntil = _unit getVariable ["FST_varrenPlanUntil", -1];
                    private _slowSince = _unit getVariable ["FST_varrenSlowSince", -1];
                    if (abs speed _unit < 1) then {if (_slowSince < 0) then {_unit setVariable ["FST_varrenSlowSince", time]; _slowSince = time}} else {_unit setVariable ["FST_varrenSlowSince", -1]; _slowSince = -1};
                    if (time < _planUntil) then {
                        if (unitReady _unit) then {_unit doMove _tpos};
                    } else {
                        if (_slowSince >= 0 && {time - _slowSince > 2}) then {
                            _unit doMove _tpos;
                            _unit setVariable ["FST_varrenPlanUntil", time + 4];
                            _unit setVariable ["FST_varrenSlowSince", -1];
                        } else {
                            _unit setDestination [_tpos, "LEADER DIRECT", true];
                        };
                    };
