    ...
    while {!isNull _unit && {alive _unit} && {local _unit} && {(_unit getVariable ["FST_varrenToken", -1]) == _token}} do {
        ...
        if (_mode > 0 && {!isNull _target} && {[_unit, _token] call FST_Gulanthar_fnc_valid}) then {
            if (time >= _nextMove) then {
                ...
                if (_mode == 2) then {
                    // Re-order only when the target moved, the order finished, or the
                    // creature stalled: every doMove restarts path planning.
                    private _tpos = getPosATL _target;
                    private _lastPos = _unit getVariable ["FST_varrenOrderPos", [0,0,0]];
                    private _lastTime = _unit getVariable ["FST_varrenOrderTime", -100];
                    if (_tpos distance2D _lastPos > 4 || {unitReady _unit} || {abs speed _unit < 1 && {time - _lastTime > 4}}) then {
                        _unit doMove _tpos;
                        _unit setVariable ["FST_varrenOrderPos", _tpos];
                        _unit setVariable ["FST_varrenOrderTime", time];
                    };
