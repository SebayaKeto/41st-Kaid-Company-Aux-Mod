// Nine matched 4v4 fights: three contexts, three unit classes.
...
private _scenarios=[["defend",140,false],["assault",140,true],["hunt",180,true]];
{
 ...
 private _round=_forEachIndex;private _cases=[];
 {
  ...
  for "_i" from 0 to 3 do{
   ...
   private _v=_enemy createUnit["FST_Trooper_P2_DC15S",_goal vectorAdd[(_i-1.5)*6,0,0],[],0,"NONE"];
 ...
 private _ready=true;
 {
  private _opponents=units(_x select 2);
  {
   removeAllWeapons _x;
   _x addBackpack "B_AssaultPack_blk";
   _x addMagazines["FST_blaster_cell_Blue",8];
   _x addWeapon "FST_DC15S";
   _x setSkill 0.5;
   if(primaryWeapon _x!="FST_DC15S" || {count primaryWeaponMagazine _x==0})then{_ready=false};
  }forEach _opponents;
  diag_log format["[CMP_OPPONENT] %1",[_round,_x select 0,_ready,_opponents apply{[typeOf _x,primaryWeapon _x,primaryWeaponMagazine _x,_x skill "general",_x skill "aimingAccuracy"]}]];
 }forEach _cases;
 if(!_ready)exitWith{diag_log "[CMP_INVALID] Opponents are not armed correctly"};
