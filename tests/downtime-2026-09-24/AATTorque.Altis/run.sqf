...
for "_repeat" from 1 to 2 do{
 {
  ...
  private _order=if(_repeat==1)then{["FST_AAT","DT_AAT_Torque15","DT_AAT_Torque30"]}else{["DT_AAT_Torque30","DT_AAT_Torque15","FST_AAT"]};
  {
   ...
   driver _v doMove _end;_v engineOn true;
   ...
   for "_i" from 1 to 35 do{
    ...
    diag_log format["[TQ_SAMPLE] %1",[_repeat,_label,_class,_i,time-_begin,_progress,_cross,speed _v,(_pos select 2)-(_origin select 2),getPosATL _v,currentCommand driver _v,isEngineOn _v,canMove _v]];
    if(_progress>=70 && {_cross<12})exitWith{_arrival=time-_begin};
   };
