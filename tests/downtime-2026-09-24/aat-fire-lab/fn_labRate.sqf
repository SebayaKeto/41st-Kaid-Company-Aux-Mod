...
if (_gunner checkAIFeature "TARGET" && {_gunner checkAIFeature "AUTOTARGET"} && {_gunner checkAIFeature "FIREWEAPON"}) then {
    ...
    if ((_v getVariable ["Q_fireAssigned",objNull])!=_target || {time>=(_v getVariable ["Q_fireNext",-1])}) then {
        _v setVariable ["Q_fireAssigned",_target];_v setVariable ["Q_fireNext",time+5];_gunner doFire _target;
    };
};
