...
for "_repeat" from 1 to 1 do{
 {
  private _case=_x;private _rows=[];
  {
   ...
   if(_variant=="driver-aware")then{driver _v disableAI "AUTOCOMBAT";driver _v setCombatBehaviour "AWARE"};
  ...
  }forEach["candidate","once","once-no-turn","interval5"];
  for "_i" from 1 to 45 do{sleep 1;{
   ...
   if(_i mod 5==0)then{diag_log format["[DT_SAMPLE] %1",[_repeat,_case,_variant,_i,getPosATL _v,getDir _v,_angle,_los,_v getVariable["Q_shots",0],currentCommand driver _v,combatBehaviour driver _v,driver _v checkAIFeature "AUTOCOMBAT",_v getVariable["BURNS_armorPulse",[]],_v getVariable["BURNS_driveDetour",[]]]]};
  }forEach _rows};
  {
