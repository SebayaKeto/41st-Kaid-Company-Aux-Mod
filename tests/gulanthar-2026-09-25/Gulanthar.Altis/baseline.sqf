...
private _canHelp=missionNamespace getVariable ["BURNS_GulantharSlopeEnabled",true] && {missionNamespace getVariable ["FST_HC_CombatTasksEnabled",true]} && {!(_g getVariable ["Q_baselineExempt",false])} && {!(_unit getVariable ["Q_baselineExempt",false])} && {(_g getVariable ["FST_HC_heldBy",-1])==-1};
...
} && {!((group _unit) getVariable ["Q_baselineExempt",false])} && {((group _unit) getVariable ["FST_HC_heldBy",-1])==-1} && {
...
} && {!(_unit getVariable ["Q_baselineExempt",false])};
